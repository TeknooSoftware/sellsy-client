...
/**
 * @link https://api.sellsy.com/documentation/methods#smarttagsgetlist
 *
 * @copyright   Copyright (c) EIRL Richard Déloge (richarddeloge@gmail.com)
 * @copyright   Copyright (c) SASU Teknoo Software (https://teknoo.software)
 *
 ...
 * @author      Richard Déloge <richarddeloge@gmail.com>
 */
class SmartTags implements DefinitionInterface
{
    public function __invoke(ClientInterface $client): CollectionInterface
    {
        $collection = new Collection($client, 'SmartTags');

        $collection->registerMethod(new Method($collection, 'getList'));
        $collection->registerMethod(new Method($collection, 'assign'));
        $collection->registerMethod(new Method($collection, 'unassign'));
