...
/**
 * @link https://api.sellsy.com/documentation/methods#customfieldsgetlist
 *
 * @copyright   Copyright (c) EIRL Richard Déloge (https://deloge.io - richard@deloge.io)
 * @copyright   Copyright (c) SASU Teknoo Software (https://teknoo.software - contact@teknoo.software)
 * @license     http://teknoo.software/sellsy-client/license/mit         MIT License
 * @author      Richard Déloge <richard@teknoo.software>
 */
class CustomFields implements DefinitionInterface
{
    public function __invoke(ClientInterface $client): CollectionInterface
    {
        $collection = new Collection($client, 'CustomFields');
        ...
        $collection->registerMethod(new Method($collection, 'recordValues'));
        $collection->registerMethod(new Method($collection, 'getGroupsList'));
        $collection->registerMethod(new Method($collection, 'createGroup'));
        $collection->registerMethod(new Method($collection, 'updateGroup'));
        $collection->registerMethod(new Method($collection, 'deleteGroup'));
